...
namespace Enjoy.Core
{
        ...
        where TModel : class
    {
        ...
        ActionResponse<TModel> SaveOrUpdate(TModel model, Func<TModel, IResponse> validate, Func<TModel, TRecord> convert);

        BaseResponse Delete(int id);

        BaseResponse Delete(ISQLQuery query);

        TRecord ConvertToRecord<TKeyType>(TModel model, Func<TRecord,TModel, TRecord> convert);
    }
}